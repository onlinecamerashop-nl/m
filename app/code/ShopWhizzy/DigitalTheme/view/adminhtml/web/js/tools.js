require(["jquery", "prototype", "mage/adminhtml/tools"], function ($) {
    ...
    $(function () {
        ...
        window.toggleValueElements = function (checkbox, container, excludedElements, checked) {
            ...
            // Trigger change on elements, excluding select-image-element
            $("select:not(.select-image-element), input, textarea, button, img", container).trigger("change");
        };
    });
});
...
require(["jquery", "spectrum"], function ($) {
    "use strict";
    $(function () {
        $(".whizzy-colorpicker, .whizzy-colorpicker .admin__field-control input[type=text]").spectrum({
            preferredFormat: "rgb",
            allowEmpty: true,
            showAlpha: true,
            showInput: true,
